...
void  ProbeRS232_InitTarget (CPU_INT32U  baud_rate)
{
    ...
    PCCR1    |=   DEF_BIT_31;                                           /* Enable UART module clock                                 */
    ...
    UCR2_1   &=  ~DEF_BIT_00;                                           /* Soft reset UART1. No UART reg accesses for 4 ipg_clocks  */
                                                                        ...
                                                                        /* ------------------- ENABLE UART1 I/Os ------------------ */
    PTE_GIUS &= ~(DEF_BIT_13 | DEF_BIT_12);
    PTE_GPR  &= ~(DEF_BIT_13 | DEF_BIT_12);

                                                                        /* --------------------- SETUP UART1 ---------------------- */
    UCR2_1   |=  (DEF_BIT_14 | DEF_BIT_12);                             /* Ignore CTS pin, set cts pin low (active)                 */
    UCR3_1   |=   DEF_BIT_02;                                           /* Configure the Rx pin multiplexor for use with the UART   */
    UCR4_1    =   0;                                                    /* Disable all interrupts                                   */
    ...
    PCCR1    |=   DEF_BIT_29;                                           /* Enable UART module clock                                 */

                                                                        /* --------------------- RESET UART3 ---------------------- */
    UCR1_3    =   0;                                                    /* DISABLE the UART to reprogram it                         */
    UCR2_3   &=  ~DEF_BIT_00;                                           /* Soft reset UART3. No UART reg accesses for 4 ipg_clocks  */

                                                                        /* ------------------- ENABLE UART3 I/Os ------------------ */
    PTE_GIUS &= ~(DEF_BIT_09 | DEF_BIT_08);
    PTE_GPR  &= ~(DEF_BIT_09 | DEF_BIT_08);

                                                                        /* --------------------- SETUP UART3 ---------------------- */
    UCR2_3   |=  (DEF_BIT_14 | DEF_BIT_12);                             /* Ignore CTS pin, set cts pin low (active)                 */
    UCR3_3   |=   DEF_BIT_02;                                           /* Configure the Rx pin multiplexor for use with the UART   */
    UCR4_3    =   0;                                                    /* Disable all interrupts                                   */

    UFCR_3    =  (1 <<  0)                                              /* Rx FIFO triggers an int. when >= 1 byte received         */
              |  (1 << 10)                                              /* Tx FIFO triggers an int. when <  1 byte in the Tx FIFO   */
              |  (5 <<  7);                                             /* Set div = 1                                              */
    ...
    UCR2_3    =   0x00004027;                                           /* UART1, Do not reset                                      */
                                                                        /* 8 Bits, 1 Stop, No Parity                                */
                                                                        /* Enable the Receiver                                      */
                                                                        /* Enable the Transmitter                                   */
                                                                        /* Ignore the RTS pin                                       */

    UCR1_3   |=   DEF_BIT_00;                                           /* Enable the UART                                          */

                                                                        /* ------------ INITIALIZE INTERRUPT FOR UART3 ------------ */
    BSP_Set_IRQ_Vector(INT_UART3, ProbeRS232_RxTxISRHandler);           /* Register the interrupt routine                           */
    BSP_IntEn(INT_UART3);                                               /* Enable the UART global interrupt                         */
#endif
}
...
#if (PROBE_RS232_CFG_COMM_SEL == PROBE_RS232_UART_3)
    if ((USR1_3 & DEF_BIT_09) == DEF_BIT_09) {                          /* Received a character?                                    */
        rx_data = (CPU_INT08U)(URXD_3 & 0x00FF);                        /* Read data.                                               */
        ProbeRS232_RxHandler(rx_data);                                  /* Call the generic Rx handler                              */
    }

    if ((USR1_3 & DEF_BIT_13) == DEF_BIT_13) {                          /* Transmitted a character?                                 */
        ProbeRS232_TxHandler();
    }
#endif
}
